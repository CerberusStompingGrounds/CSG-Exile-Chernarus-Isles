private ["_num", "_side", "_pos", "_OK", "_difficulty", "_AICount", "_group", "_type", "_launcher", "_staticGuns", "_crate1", "_vehicle", "_pinCode", "_class", "_crate_loot_values1", "_missionAIUnits", "_missionObjs", "_msgStart", "_msgWIN", "_msgLOSE", "_missionName", "_markers", "_time", "_added", "_cleanup", "_baseObjs", "_crate_weapons", "_crate_weapon_list", "_crate_items", "_crate_item_list", "_crate_backpacks", "_PossibleDifficulty"];
...
switch (_difficulty) do
{
	...
	case "moderate":
	{
		...
		_crate_weapons 		= (1 + (round (random 1)));
		_crate_items 		= (10 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 1)));
	};
	case "difficult":
	{
	...
	default
	{
		...
		_crate_items 		= (20 + (round (random 5)));
...
_msgStart = ['#DE5021',"A survivor camp has been raided, get your revenge!"];
_group =
[
	_pos,					// Position of AI
...
[
	[
		...
		[(_pos select 0) -(10-(random 2)),(_pos select 1)+(10-(random 2)),0]
	],
	_group,
	"assault",
	"static",
...
if (_difficulty isEqualTo "hardcore") then
{
	...
	_vehicle = ["Exile_Bike_QuadBike_Guerilla01",_pos getPos [30, random 360],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#DE5021',format ["Survivors have got their hands on the loot, entry code for Quad is %1...",_pinCode]];
} else
{
	_vehicle = ["Exile_Bike_QuadBike_Guerilla01",_pos getPos [30, random 360]] call DMS_fnc_SpawnNonPersistentVehicle;
	_msgWIN = ['#DE5021',"Survivors have got their hands on the loot"];
};
...
_msgLOSE = ['#DE5021',"The militia burned the camp..."];
// Define mission name (for map marker and logging)
_missionName = "Survivor Camp";
