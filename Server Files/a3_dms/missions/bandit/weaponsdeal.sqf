...
switch (_difficulty) do
{
	...
	case "moderate":
	{
		...
		_crate_weapons 		= (6 + (round (random 1)));
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (8 + (round (random 2)));
	...
	default
	{
		...
		_crate_weapons 		= (10 + (round (random 3)));
		_crate_items 		= (10 + (round (random 5)));
		_crate_backpacks 	= (2 + (round (random 1)));
	};
};
//_msgStart = ['#DE5021',format["A %1 militia weapons deal has gone wrong!",_difficulty]];
_msgStart = ['#DE5021',"A militia weapons deal has gone wrong!"];
_group =
[
	_pos,					// Position of AI
...
if (_difficulty isEqualTo "hardcore") then
{
	...
	_vehicle = ["rhs_gaz66o_msv",_pos getPos [30, random 360],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#DE5021',format ["Survivors have got the weapons, entry code for GAZ-66 is %1...",_pinCode]];
} else
{
	_vehicle = ["rhs_gaz66o_msv",_pos getPos [30, random 360]] call DMS_fnc_SpawnNonPersistentVehicle;
	_msgWIN = ['#DE5021',"Survivors have got the guns"];
};
...
_msgLOSE = ['#DE5021',"The militia recovered it all..."];
// Define mission name (for map marker and logging)
_missionName = "Weapons Deal";
