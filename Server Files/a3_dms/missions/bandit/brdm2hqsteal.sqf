/*
	BRDM-2 HQ Mission with new difficulty selection system
...
switch (_difficulty) do
{
	case "easy":
	{
		_msgWIN = ['#DE5021',"Survivors killed everyone and made off with the BRDM-2 HQ"];
		_AICount = (4 + (round (random 4)));
		_crate_weapons 		= (1 + (round (random 2)));
		_crate_items 		= (4 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 2)));
		_vehicle = ["Exile_Car_BRDM2_HQ",[(_pos select 0) -30, (_pos select 1) -30]] call DMS_fnc_SpawnNonPersistentVehicle;
	};
	case "moderate":
	{
		...
		{
			_vehicle = ["Exile_Car_BRDM2_HQ",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#DE5021',format ["Survivors killed everyone and made off with the BRDM-2 HQ, entry code %1...",_pinCode]];
		};
	};
	case "difficult":
	{
...
//_msgStart = ['#DE5021',format["A BRDM-2 HQ is parked at a small %1 base! Go kill them and steal it",_difficulty]];
_msgStart = ['#DE5021',"A BRDM-2 HQ is parked at a small base! Go kill them and steal it"];
_group =
[
	_pos,					// Position of AI
...
_msgLOSE = ['#DE5021',"The attackers drove off with the BRDM-2 HQ escaped attack"];
// Define mission name (for map marker and logging)
_missionName = "BRDM-2 HQ Steal";
// Create Markers
_markers =
[
	_pos,
