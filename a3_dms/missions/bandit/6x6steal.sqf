...
_msgStart = ['#DE5021',"Militia with truck have broken down. Go kill them and steal it"];
_group =
[
	_pos,					// Position of AI
...
if (_difficulty isEqualTo "easy") then {
									if ((round (random 1)) isEqualTo 0) then {
																				...
																				_msgWIN = ['#DE5021',"Survivors killed everyone and made off with the offroad"];
																			} else
																			{
																				_pinCode = (1000 +(round (random 8999)));
																				_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
																				_msgWIN = ['#DE5021',format ["Survivors killed everyone and made off with the offroad, entry code %1...",_pinCode]];
																			};
										} else
										{
											_pinCode = (1000 +(round (random 8999)));
...
_msgLOSE = ['#DE5021',"The attackers drove off in the truck..."];
// Define mission name (for map marker and logging)
_missionName = "6X6 Steal";
// Create Markers
_markers =
[
	_pos,
