...
switch (_difficulty) do
{
	case "easy":
	{
		_msgStart = ['#DE5021',"An unarmed M113 is parked at a small easy base! Go kill them and steal it"];
		_msgWIN = ['#DE5021',"Survivors killed everyone and made off with the M113"];
		_AICount = (4 + (round (random 4)));
		_crate_weapons 		= (1 + (round (random 1)));
		_crate_items 		= (4 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 2)));
		_vehicle = ["rhsusf_m113_usarmy_unarmed",[(_pos select 0) -30, (_pos select 1) -30]] call DMS_fnc_SpawnNonPersistentVehicle;
	};
	case "moderate":
	{
		_msgStart = ['#DE5021',"An unarmed M113 is parked at a small moderate base! Go kill them and steal it"];
		...
		{
			_vehicle = ["rhsusf_m113_usarmy_unarmed",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#DE5021',format ["Survivors killed everyone and made off with the M113, entry code %1...",_pinCode]];
		};
	};
	case "difficult":
	{
		_msgStart = ['#DE5021',"A M113 is parked at a small difficult base! Go kill them and steal it"];
		_msgWIN = ['#DE5021',format ["Survivors killed everyone and made off with the M113, entry code %1...",_pinCode]];
	...
	case "hardcore":
	{
		_msgStart = ['#DE5021',"A M113 is parked at a small hardcore base! Go kill them and steal it"];
		_msgWIN = ['#DE5021',format ["Survivors killed everyone and made off with the M113, entry code %1...",_pinCode]];
...
_msgLOSE = ['#DE5021',"The attackers drove off with the M113 and the base escaped attack..."];
// Define mission name (for map marker and logging)
_missionName = "M113 Steal";
// Create Markers
_markers =
[
	_pos,
