...
if ((isNil "marketMeanValues") && !UK111_marketMeanValuesDone) then {
	UK111_marketMeanValuesDone = true;
	["meanValueRequest",["myLittlePony"]] call ExileClient_system_network_send;
};
...
switch (_option) do {
    ...
    case ("LoadRight"): {
        ...
        switch (_location) do {
            case 0: {
                ...
                { ctrlShow [_x,false]; } forEach [21020,21021,21022,21023,21031,21032];
                {
                    _text = "";
                    ...
                    if ((_x select 1) isEqualTo 1) then {
                        ...
                        if (!(_configName isEqualTo "CfgVehicles") || (_configName isEqualTo "CfgVehicles" && _type isEqualTo 3)) then {
                            ...
                            if (_sellersUID isEqualTo (getPlayerUID player)) then { _price = str(0); };
                            ...
                            if (_clientMoney < parseNumber(_price)) then { lbSetColorRight [21017,_index, [0.8,0,0,1]]; };
                ...
                private ["_text","_ClassName","_listingID","_price","_configName","_name","_index","_fuel","_health","_sellerUID","_type"];
                {
                    ...
                    if ((_x select 1) isEqualTo 1) then {
                        ...
                        if (_configName isEqualTo "CfgVehicles" && _type != 3) then {
                            ...
                            _sellersUID = _x select 4;
                            if (_sellersUID isEqualTo (getPlayerUID player)) then { _price = str(_rekeyCost); };
    ...
    case ("LoadLeft"):
    {
        ...
        MarXet_VehicleObjectArray = [];
        switch (_location) do {
        	case 0: { _items = [player, true] call ExileClient_util_playerEquipment_list; };
        	case 1: { _items = (uniformContainer player) call ExileClient_util_containerCargo_list; };
        	case 2: { _items = (vestContainer player) call ExileClient_util_containerCargo_list; };
        	case 3: { _items = (backpackContainer player) call ExileClient_util_containerCargo_list; };
        	default {
            ...
            if (_location isEqualTo 0) then {
                {
                    if (_className isEqualTo (_x select 0)) then {
                        ...
                        if !((count _items) isEqualTo 0) then { _canList = false; };
                    };
                }
                forEach [[uniform player, uniformContainer player],[vest player, vestContainer player],[backpack player, backpackContainer player]];
            };
            if (_canList) then {
    ...
    case ("LoadCenter"): {
        ...
        switch (_this select 1) do {
            case 0: {
                ...
                { ctrlShow [_x,false]; } forEach [21020,21021,21022,21023,21024,21031,21032];
                ...
                if !(_dataString isEqualTo "") then {
                    ...
                    if (_location isEqualto 1) then {
                        ...
                        { ctrlShow [_x,true]; } forEach [21020,21021,21022,21023,21031,21032];
                        ctrlSetText [21032,""];
                    ...
                    if ( _sellerUID isEqualTo (getPlayerUID player)) then { MarXet_BuyerIsSeller = true; };
                        ...
                        if !(_location isEqualTo 1) then {
                            ...
                            try {
                                switch (_leftlocation) do {
                                    case 0: {
                                        if !([player,_itemClassName] call ExileClient_util_playerCargo_canAdd) then { throw 0; };
                                    };
                                    case 1: {
                                        if !(player canAddItemToUniform _itemClassName) then { throw 0; };
                                    };
                                    case 2: {
                                        if !(player canAddItemToVest _itemClassName) then { throw 0; };
                                    };
                                    case 3: {
                                        if !(player canAddItemToBackpack _itemClassName) then { throw 0; };
                                    };
                                    case 4: { throw 1; };
                                };
                                _purchaseBtn ctrlSetText "Purchase";
            ...
            case 1: {
                ...
                MarXet_TempListingClassname = "";
                _leftLB = (_display displayCtrl 21018);
                { ctrlShow [_x,false]; } forEach [21020,21021,21022,21023,21012,21013,21014];
                _priceEditBox = _display displayCtrl 21011;
                ctrlEnable [21011,false];
                _priceEditBox ctrlSetText "";
                ...
                if !(_dataString isEqualTo "") then {
					...
					//Average sell price
					private ["_forgetIt", "_soldBefore", "_thatName", "_itemMeanValue"];
					_forgetIt = false;
					if ((isNil "marketMeanValues") || (marketMeanValues isEqualTo [])) then {
						_forgetIt = true;
					};
					if (!_forgetIt) then {
						_soldBefore = false;
						_thatName = _dataArray select 1;
						{
							if ((_dataArray select 0) isEqualTo (_x select 0)) exitWith {
								_itemMeanValue = (_x select 1) / (_x select 2);
								["InfoTitleAndText",["Marketplace",format["%1<br/>Average sell price: <t color='#ff0000'>%2</t> <img image='\exile_assets\texture\ui\poptab_inline_ca.paa' size='24'/><br/>Sold: <t color='#ff0000'>%3</t> pcs.",_thatName,round(_itemMeanValue),(_x select 2)]]] call ExileClient_gui_toaster_addTemplateToast;
								_soldBefore = true;
							};
						} forEach marketMeanValues;
						if (!_soldBefore) then {
							["InfoTitleAndText",["Marketplace",format["%1<br/>Average sell price: <t color='#ff0000'>None sold</t>",_thatName]]] call ExileClient_gui_toaster_addTemplateToast;
						};
					};
					//Average sell price
                    ...
                    if (_location isEqualTo 4) then {
                        ...
                        ctrlSetText [21021,_healthText];
                        ctrlSetText [21023,_fuelText];
                        { ctrlShow [_x,true]; } forEach [21021,21023,21020,21022];
                    } else {
    ...
    case ("buttonPressed"): {
        ...
        switch (_this select 1) do {
            ...
            case 1: {
				...
				_sellPrice = parseNumber(ctrlText 21011);
				
                if (_sellPrice isEqualTo 0) exitWith { ["ErrorTitleOnly",["Please set a list price"]] call ExileClient_gui_toaster_addTemplateToast; MarXet_ListingArray = nil; };
				if (_sellPrice > 100000) exitWith { ["ErrorTitleOnly",["You cannot set a sell price over 100k"]] call ExileClient_gui_toaster_addTemplateToast; MarXet_ListingArray = nil; };
				
				_traderPrice = getNumber(missionConfigFile >> "CfgExileArsenal" >> (MarXet_TempListingClassname select 0) >> "price");
				if (_traderPrice isEqualTo 0) then { _traderPrice = 100000; };
				if (_sellPrice > _traderPrice) exitWith {
					["ErrorTitleOnly",[format["You cannot set the sell price over trader price(%1)",_traderPrice]]] call ExileClient_gui_toaster_addTemplateToast;
    ...
    case ("DisplayNotification"): {
        ...
        switch (_this select 1) do {
            case 0: {
                ...
                { ctrlEnable [_x,false]; } forEach [21011,21014,21024,21015,21016,21017,21018,21019,21025,21007,21008,21032,21033];
                ...
                {
                    ctrlShow [_x,true];
                    if !(_x isEqualTo 21029) then { ctrlEnable [_x,true]; };
                ...
                switch (_textOption) do {
                    case 0: {
                        _textCtrl ctrlSetStructuredText parseText format["<t size='1'>Looks like you are the person who listed this vehicle!<br/>Unfortunately, the vehicle has to be re-keyed.<br/>There is a <t color='#e32636'>%1</t> poptab re-keying fee on this vehicle.<br/>Are you sure you want to continue?</t>",MarXet_Poptab];
                    };
                    case 1: {
            ...
            case 1: {
                { ctrlEnable [_x,true]; } forEach[21014,21024,21015,21016,21017,21018,21019,21025,21007,21008,21032,21033];
                {
                    ctrlShow [_x,false];
                    if !(_x isEqualTo 21029) then { ctrlEnable [_x,false]; };
                } forEach[21029,21026,21027,21028,21030];
