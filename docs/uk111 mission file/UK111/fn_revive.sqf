...
if (ExilePlayerInSafezone) exitWith { ["ErrorTitleOnly",["You cannot revive inside the safezone!"]] call ExileClient_gui_toaster_addTemplateToast; };
if !(isPlayer _target) exitWith { ["ErrorTitleOnly",["There is no pulse... RIP!"]] call ExileClient_gui_toaster_addTemplateToast; };

if (_target getVariable "revivePermitted") then {
	if (magazines player find "Exile_Item_Defibrillator" >= 0) then {
		...
		if (_target getVariable["REVIVE",true]) then {
			...
			if (_secondsRemaining >= 17) then {
				_target setVariable["reviveAntiDupe",-1,true]; 
				["UK111ReviveRequest",[netID _target]] call ExileClient_system_network_send;
			} else {		
				["ErrorTitleOnly",[format["RIP %1! He is gone!",_targetname]]] call ExileClient_gui_toaster_addTemplateToast;
			};
		} else {		
			["ErrorTitleOnly",[format["RIP %1! He suffered a fatal injury!",_targetname]]] call ExileClient_gui_toaster_addTemplateToast;
		};
	}; 
} else {		
	["ErrorTitleOnly",[format["RIP %1! He suffered a fatal injury!",_targetname]]] call ExileClient_gui_toaster_addTemplateToast;
